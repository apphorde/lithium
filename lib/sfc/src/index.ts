...
export function getComponentCode(name: string, { template, setup, shadowDom }, withExport = true) {
...
export function parseSFC(source: string) {
  ...
  if (templateNode) {
    sfc.template = JSON.stringify(templateNode.children.map(serialize).join(''));
  }
