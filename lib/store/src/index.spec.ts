...
describe('store', () => {
  ...
  it('should not dispatch events until a state transition is commited', async () => {
    ...
    await store.transaction(async () => {
      await store.dispatch('increment', 1);
      assert.strictEqual(1, store.get(count), 'increment inside transaction has wrong value');

      await store.dispatch('increment', 10);
      assert.strictEqual(11, store.get(count), 'increment inside transaction has wrong value');
